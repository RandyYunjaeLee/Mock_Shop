...
<html>
<head>
<title>Totally Legit Game Keys Order Processing</title>
</head>
<body>
		...
		if (custId == null || custId.equals(""))
			...
			try (Connection con = DriverManager.getConnection(url, uid, pw);) {
				...
				if (!rst.next())
					...
					String sqlTaxes = "SELECT stateTax FROM taxes WHERE stateId = ?";
					pstmt = con.prepareStatement(sqlTaxes);
					pstmt.setString(1, state);
					rst = pstmt.executeQuery();
					rst.next();
					double taxAmount = rst.getDouble(1);
					total = total * (100+taxAmount)/100;
					double taxPrice = total * (taxAmount/100);
					out.println("<tr><td colspan=\"4\" align=\"right\"><b>Taxes</b></td>"
							+ "<td aling=\"right\">" + currFormat.format(taxPrice) + "</td></tr>");
					...
					pstmt.executeUpdate();
					sql = "SELECT paymentType FROM paymentmethod WHERE customerId = ?";
					pstmt = con.prepareStatement(sql);
					pstmt.setString(1, custId);
					ResultSet r = pstmt.executeQuery();
					if(r.next()){
						String payment = r.getString("paymentType");
						out.println("<h1>Paid via:" + payment + "</h1>");
					}
